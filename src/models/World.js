  ...
  checkMastersVoiceChannels(user_id) {
    ...
    var delayed_execution = function() {
      ...
      for ( var i=0;i<leave_servers.length;i++) {
        ...
        s.talk('Herkese bye', null, function() { s.release(); });
      }
    };
 ...
 * * */
  ...
  renderPresenceFollow() {
    ...
    return Object.keys(bot.servers).length + " server, " + cmds.command_char + "takip";
  };

  renderPresenceHelp() {
    var cmds = require("@commands");
    return Object.keys(bot.servers).length + " server, " + cmds.command_char + "yardımı";
  };

  renderPresenceCounts() {
    var w = this;
    var s = Object.keys(bot.servers).length + " server, " + w.getActiveServersCount() + " aktif";
    return s;
  };
