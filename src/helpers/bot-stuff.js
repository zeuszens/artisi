...
class BotStuff {
  ...
  constructor(shard_number, total_shards) {
    ...
    Common.out('Toplam ID#' + shard_number + '. Oluyor ' + total_shards);
  ...
  getNickFromUserId(channel_id, user_id) {
    ...
    if (!bot.channels[channel_id])
      Common.out("getNickFromUserId(): Bulamadım " + channel_id + ", " + user_id);
    else if (!bot.servers[bot.channels[channel_id].guild_id])
      Common.out("getNickFromUserId(): Bulamadım " + bot.channels[channel_id].guild_id + " for channel " + channel_id);
    else if (!bot.servers[bot.channels[channel_id].guild_id].members[user_id])
      Common.out("getNickFromUserId(): Bulamadım " + bot.channels[channel_id].guild_id + ", " + user_id);
    else
