...
class Mute extends Command {
  ...
  execute ({input, server, world}) {
    if (server.getUserSetting(input.user_id, 'sustu'))
      return input.il8nResponse('mute.alreadymuted');

    server.addUserSetting(input.user_id,'sustu',true);
...
class UnMute extends Command {
  ...
  execute ({input, server, world}) {
    ...
    if (!server.getUserSetting(user_id, 'sustu'))
      return input.il8nResponse('unmute.alreadyunmuted');

    server.addUserSetting(user_id,'sustu',false);
