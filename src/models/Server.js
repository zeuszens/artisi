...
class Server {
  ...
  joinVoiceChannel(channel_id, callback) {
    ...
    if (server.connecting) return Common.error('joinVoiceChannel(' + channel_id + '): iki kez bağlanmayı denedim!');
    server.connecting = true;

    if (!server.isServerChannel(channel_id)) return Common.error('joinVoiceChannel(' + channel_id + ') ee yanlış server za');
  ...
  neglected() {
    ...
    if (server.inChannel()) {
      server.talk("Kötü hissettim çıkıyom", null, neglectedrelease);
    } else {
  ...
  shutdown() {
    ...
    if (server.inChannel()) {
      server.talk("Server GG", null, function () {
        server.release();
      });
    }
    else {
